...
from ..platform._base import PatternId, ToggleState
...
class GroupManager(BasePage):
    ...
    def _press_key(self, key_code: int, hold_time: float = 0.1) -> None:
        """按下并释放一个虚拟按键。"""
        platform.input.key_down(key_code)
        time.sleep(hold_time)
        platform.input.key_up(key_code)

    def _send_ctrl_combo(self, key_code: int, settle_time: float = 0.3) -> None:
        """发送 Ctrl+<key> 组合键并短暂等待 UI 更新。"""
        platform.input.send_combo(platform.input.VK_CONTROL, key_code, settle_time)
    ...
    def _scroll_list(self, ctrl, delta: int, steps: int, step_delay: float, settle_time: float) -> None:
        ...
        platform.input.set_cursor(cx, cy)
        for _ in range(steps):
            platform.input.mouse_scroll(delta, 1, step_delay)
    ...
    def _find_announcement_window(self) -> Optional[dict]:
        ...
        def enum_callback(hwnd, results):
            title = platform.window_manager.get_title(hwnd)
            if '公告' in title:
    ...
    def _get_announcement_popup(self):
        ...
        popup = platform.automation.control_from_handle(hwnd)
        if not popup:
    ...
    def _find_and_activate_button(self, popup, button_name: str) -> bool:
        ...
        for tab_count in range(20):
            self._press_key(platform.input.VK_TAB, hold_time=0.15)
            time.sleep(0.3)
            ...
            for ctrl in all_controls:
                if ctrl.Name == button_name:
                    ...
                    for key_name, key_code in [("Space", platform.input.VK_SPACE), ("Return", platform.input.VK_RETURN)]:
    ...
    def get_group_members(self, group_name: str) -> list:
        ...
        for i in range(10):
            self._press_key(platform.input.VK_TAB, hold_time=0.05)
            time.sleep(0.3)
            ...
            if focused and '查看更多' in (focused.Name or ''):
                ...
                self._press_key(platform.input.VK_RETURN)
                time.sleep(1)
                break
        else:
    ...
    def _has_existing_announcement(self, popup, max_tabs: int = 15) -> bool:
        ...
        for _ in range(max_tabs):
            self._press_key(platform.input.VK_TAB)
            time.sleep(0.2)
    ...
    def get_group_nickname(self, group_name: str) -> Optional[str]:
        ...
        try:
            ...
            self._press_key(platform.input.VK_ESCAPE)
            if nickname:
    ...
    def _set_toggle_in_detail_panel(self, group_name: str, control_name: str, enable: bool) -> bool:
        ...
        self._press_key(platform.input.VK_SPACE)
        time.sleep(0.5)

        # 第5步：重新读取焦点以验证
        new_ctrl = platform.automation.get_focused_control()
        if new_ctrl:
