...
![普通 AI 回复示例](docs/images/readme/chat-normal-ai.png)

机器人只处理配置群里 @ 当前登录昵称的消息。多个群同时触发时会进入同一个发送队列，按顺序回复，避免抢焦点和串群。

```text
用户A：@机器人 今天项目测试环境要注意什么？
机器人：@用户A 可以先确认服务状态、依赖配置和最近错误日志...
```
...
带 OpenClaw 前缀的消息会交给本地 agent。引用文件卡片时，机器人会把附件路径传给 OpenClaw，并在处理完成后把生成文件发回群里。

```text
用户A：引用「示例表单_10.docx」并发送：@机器人 /c 文件名称序号改成 11 发给我
机器人：@用户A 已改成 11 了，文件路径：/Users/.../.openclaw/workspace/.wx4py_files/示例表单_11.docx
...
- 要让 OpenClaw 处理文件，建议直接引用微信文件卡片再发送命令，例如 `@机器人 /c 文件名称序号改成 12 发给我`。单独说“这个文件”但没有引用气泡时，机器人通常无法可靠判断是哪一个文件。
...
8. 引用一个 docx 文件并发送 `@机器人 /c 文件名称序号改成 12 发给我`，确认 OpenClaw 命令里带有附件信息，且最终发送的是新生成文件。
9. 在上一条文件任务后发送 `@机器人 /c 很好`，确认 OpenClaw 命令里不再携带上一条附件。
10. 让 OpenClaw 回复包含 `表单号_12.docx，...` 这类中文标点后的文件名，确认机器人仍能解析并发送文件。

## 最近更新

- **2026-05-06**：完善 OpenClaw 文件引用和文件回传链路。支持从微信引用气泡解析附件、传入 OpenClaw、识别生成文件并自动发送；修复纯文本消息误继承旧附件、真实引用消息丢附件、输入文件副本被误发、中文标点后文件名无法解析等问题。README 新增功能示例、文件链路和 macOS 操作注意事项。
