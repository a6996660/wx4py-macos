...
# uiautomation 仅在 Windows 平台导入，macOS 上使用 platform 抽象层代替
def _get_uia():
    """条件导入 uiautomation（仅 Windows）。
    
    用于托盘 UIA 代码的平台守卫——macOS 无等效托盘结构，
    所有托盘相关 UIA 调用均由 platform 抽象层处理。
    """
    if sys.platform == "win32":
        from . import uiautomation as _uia
        return _uia
    return None
...
def _should_restart_after_registry_fix(registry_status: str) -> bool:
    ...
    Windows: RunningState 从 0 修复为 1 时需要重启。
    macOS: 辅助功能权限刚被授权时可能需要重启。
    """
    return registry_status in ("fixed_zero", "granted_after_prompt")


def _is_main_window_class(cls: str) -> bool:
    """判断窗口类名是否属于微信主窗口。

    Windows: 类名包含 'MainWindow'。
    macOS: 类名包含 'AXWindow'（从 AXRole 获取）。
    """
    if not cls:
        return False
    return 'MainWindow' in cls or 'AXWindow' in cls
...
class WeChatWindow:
    ...
    def _find_wechat_tray_item(self):
        """查找微信托盘图标，必要时自动展开隐藏区（仅 Windows 有效）。"""
        uia = _get_uia()
        if uia is None:
            return None
    ...
    def _restore_via_tray_icon(self) -> bool:
        """通过托盘图标恢复微信。"""
        restored = platform.accessibility.restore_from_tray(self._hwnd)
        if restored:
    ...
    def _activate_hwnd(self, hwnd: int) -> bool:
        ...
        if not platform.window_manager.is_visible(hwnd):
            ...
            deadline = time.time() + _TRAY_OVERFLOW_WAIT_SECONDS
            while time.time() < deadline:
                refreshed_hwnd = platform.window_manager.find_wechat_window()
                if refreshed_hwnd and platform.window_manager.is_visible(refreshed_hwnd):
                        ...
                        try:
                            ...
                            try:
                                ...
                                if rect:
                                    ...
                                    platform.input.mouse_down(x, y)
                                    time.sleep(0.1)
                                    platform.input.mouse_up(x, y)
                            except Exception:
                                pass
    ...
    def _wait_for_main_window(self, timeout: int = 20):
        ...
        for i in range(timeout):
            ...
            if hwnd:
                cls = platform.window_manager.get_class(hwnd)
                if _is_main_window_class(cls):
    ...
    def _restart_and_reconnect(self):
        ...
        """
        restarted = platform.process.restart_process(self._hwnd)
        self.disconnect()
        if not restarted:
    ...
    def connect(self) -> bool:
        ...
        """
        # 第1步：修复辅助功能环境。
        # 检查并配置平台相关的辅助功能设置（Windows注册表 + 屏幕阅读器标志，
        # macOS 辅助功能权限）。
        logger.info("正在检查辅助功能环境...")
        registry_status = "unchanged"
        try:
            registry_status = platform.accessibility.ensure_accessibility_environment()
            ...
            elif registry_status == "granted_after_prompt":
                logger.info("macOS 辅助功能权限已获得授权")
            elif registry_status == "unauthorized":
                logger.warning("macOS 辅助功能权限未授权，请在系统偏好设置中授权")
            else:
                logger.debug("辅助功能环境已正确设置")
        except Exception as e:
            logger.warning(f"辅助功能环境检查失败: {e}")
        
        # 只有辅助功能环境刚被修复时，才要求重启微信使修复生效。
        ...
        if settings_changed:
            logger.warning("检测到辅助功能环境变更，正在重启微信以使修复生效...")
            self._restart_and_reconnect()
            return True

        # 第5步：检测是否仍停留在登录界面。
        cls = platform.window_manager.get_class(self._hwnd)
        if not _is_main_window_class(cls):
    ...
    def title(self) -> str:
        """获取窗口标题"""
        if self._hwnd:
            return platform.window_manager.get_title(self._hwnd)
        return ""
    ...
    def class_name(self) -> str:
        """获取窗口类名"""
        if self._hwnd:
            return platform.window_manager.get_class(self._hwnd)
        return ""
